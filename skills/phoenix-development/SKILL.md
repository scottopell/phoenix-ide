...
6. Poll the source comment with a bounded wait for the reducer reaction lifecycle: 👀 means processing, 🚀 means accepted, and 😕 means rejected. After editing a comment, first require a fresh 👀 observation before accepting a later 🚀 or 😕. On 😕, inspect the roadmap reducer Actions log for the validation or application error and report it; the snapshot marker alone does not prove acceptance.
