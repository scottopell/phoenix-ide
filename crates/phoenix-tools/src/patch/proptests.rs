...
/// Generate non-empty indentation (for tests that require actual indentation)
fn arb_nonempty_indent() -> impl Strategy<Value = String> {
    prop_oneof![
        Just("  ".to_string()),
        Just("    ".to_string()),
        Just("\t".to_string()),
...
///
/// This is critical for testing string truncation code paths that might
/// panic if they slice at byte boundaries that fall mid-character.
fn arb_unicode_content() -> impl Strategy<Value = String> {
    prop_oneof![
        // ASCII baseline
        "[a-zA-Z0-9 \n\t]{1,200}",
        // Box drawing characters (common in markdown tables/diagrams)
        "[a-z \n\u{2500}\u{2502}\u{250C}\u{2510}\u{2514}\u{2518}]{1,100}",
        // Mixed ASCII and common symbols
        "[a-z \u{2705}\u{2714}\u{2716}]{1,50}",
    ]
}

/// Generate long content with multi-byte UTF-8 chars near truncation boundaries
///
/// Creates content > 2000 bytes with multi-byte chars positioned to potentially
/// land at byte 2000 (the truncation point in `is_autogenerated`).
fn arb_unicode_content_long() -> impl Strategy<Value = String> {
    // Strategy: generate a base of ASCII, then insert multi-byte chars at various positions
    (
        "[a-z]{2500,3000}",
        prop::collection::vec(0usize..2500, 10..50),
    )
        .prop_map(|(base, positions)| {
            let mut chars: Vec<char> = base.chars().collect();
            let multi_byte_chars = ['\u{2500}', '\u{4E2D}', '\u{2714}']; // 3-byte chars
            for (i, pos) in positions.into_iter().enumerate() {
                if pos < chars.len() {
                    chars[pos] = multi_byte_chars[i % multi_byte_chars.len()];
                }
            }
...
// ============================================================================
// Invariant 7: Unicode content handling
//
// String truncation must respect UTF-8 char boundaries to avoid panics.
// This tests the is_autogenerated function which truncates at ~2000 bytes.
// ============================================================================

proptest! {


    /// Test that is_autogenerated handles arbitrary unicode without panicking
    #[test]
    fn prop_is_autogenerated_handles_unicode(content in arb_unicode_content()) {
        // Should not panic regardless of content
        let _ = super::planner::is_autogenerated(&content);
    }


    /// Test that our truncation helper is safe
    #[test]
    fn prop_truncate_to_char_boundary_safe(
        content in arb_unicode_content(),
        max_bytes in 0usize..500
    ) {
        let truncated = truncate_to_char_boundary(&content, max_bytes);
        // Result should be valid UTF-8 (it is, since it's a String)
        // Result length should be <= max_bytes
        prop_assert!(truncated.len() <= max_bytes || content.len() <= max_bytes);
        // Result should be a prefix of the original
        prop_assert!(content.starts_with(&truncated));
    }
}
...
proptest! {
    ...
    /// Test is_autogenerated with long content containing multi-byte chars
    /// near the 2000-byte truncation boundary. 32 cases suffice — the generator
    /// always produces >2000 bytes so every case exercises the truncation path.
    #[test]
    fn prop_is_autogenerated_handles_long_unicode(content in arb_unicode_content_long()) {
        prop_assume!(content.len() > 2000);
        let _ = super::planner::is_autogenerated(&content);
    }
}
...
#[test]
fn test_is_autogenerated_with_box_drawing() {
    // Box drawing char ─ is 3 bytes (E2 94 80)
    let content = "a".repeat(1998) + "\u{2500}" + "more text";
    // Byte 1998 + 3 = 2001, so truncation at 2000 would be mid-character
    // This should not panic
    let _ = super::planner::is_autogenerated(&content);
}

#[test]
fn test_is_autogenerated_with_emoji() {
    // Emoji 😀 is 4 bytes (F0 9F 98 80)
    let content = "a".repeat(1997) + "\u{1F600}" + "more text";
    // Byte 1997 + 4 = 2001, truncation at 2000 would be mid-emoji
    let _ = super::planner::is_autogenerated(&content);
}

#[test]
fn test_is_autogenerated_with_cjk() {
    // CJK char 中 is 3 bytes (E4 B8 AD)
    let content = "a".repeat(1999) + "\u{4E2D}" + "more text";
    // Byte 1999 + 3 = 2002, truncation at 2000 would be mid-character
    let _ = super::planner::is_autogenerated(&content);
}

#[test]
fn test_is_autogenerated_detects_marker_after_unicode() {
    // Ensure detection still works when unicode precedes the marker
    let content = "\u{2500}".repeat(100) + "\n// Code generated by tool\n" + &"x".repeat(100);
    assert!(super::planner::is_autogenerated(&content));
}

#[test]
fn test_is_autogenerated_short_content() {
    // Content shorter than truncation limit
    let content = "short \u{1F600} content";
    let _ = super::planner::is_autogenerated(content);
}

#[test]
fn test_is_autogenerated_exactly_at_boundary() {
    // Content exactly 2000 bytes
...
#[test]
fn test_truncate_helper_edge_cases() {
    // Empty string
    ...
    // Truncate mid-emoji
    let s = "a\u{1F600}b"; // 'a' (1) + 😀 (4) + 'b' (1) = 6 bytes
