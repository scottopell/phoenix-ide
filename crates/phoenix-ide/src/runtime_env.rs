...
//!
//! Historically `std::env::var("HOME")`, `CODEX_HOME`, and
//! `std::env::temp_dir()` were read inline at a dozen call sites, each with
//! its own fallback (`/tmp`, `/root`, `.`, …). [`PhoenixRuntimeEnvironment`]
//! resolves all of them once at startup, behind a typed surface, so every
//! subsystem agrees on where Phoenix's data lives.
//!
//! Construction:
...
//! - tests: [`PhoenixRuntimeEnvironment::with_root`] — no environment reads;
//!   everything is derived under a caller-supplied directory so tests need
//!   not mutate process env vars.
//!
...
impl PhoenixRuntimeEnvironment {
    ...
    /// `phoenix_home/prod.log` — the production log file. Part of the typed
    /// surface; the only current consumer (`src/bin/monitor.rs`) is a
    /// separate binary with no shared lib crate, so this is unused in the
    /// main crate today — kept for parity with `main.rs`'s prod logging path
    ...
    /// (matches [`crate::skills::builtin::EXTRACT_SUBDIR`]).
    #[must_use]
    pub fn builtin_skills_dir(&self) -> PathBuf {
        self.phoenix_home()
            .join(crate::skills::builtin::EXTRACT_SUBDIR)
    }
