...
pub(crate) async fn get_conversation_pr_status(
    ...
    let selection = active_selection_target_for_scope(&db, &work_scope).await?;
    ...
    response.selection = selection_envelope_for_scope(&db, &work_scope).await?;
