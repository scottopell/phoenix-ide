...
async fn cached_pr_summary_for_conversation(
    ...
    let Some(scope) = conv.attached_work_scope_id.as_ref() else {
        return Ok(None);
    };
    Ok(state
        .runtime
...
async fn cached_pr_summaries_for_conversations(
    ...
    let scopes: Vec<_> = conversations
        .iter()
        .filter_map(|conv| conv.attached_work_scope_id.clone())
        .collect();
...
async fn get_system_prompt(
    ...
    let system_prompt = if is_coordinator {
        let coordinator_catalog = crate::skills::AuthenticatedCoordinatorSkillCatalog::discover(
...
async fn continue_conversation(
    ...
    let (outcome, intent) = state
        ...
        .map_err(|e| match e {
            ...
            DbError::ProductConversationUnavailable(_) => AppError::Conflict(Box::new(
                ConflictErrorResponse::new(
                    "History ProductConversations cannot be continued; create a separate Open follow-up.",
                    "product_conversation_in_history",
                ),
            )),
...
pub(super) async fn reopen_bash_after_failed_lifecycle_mutation(
    ...
    if !crate::runtime::conversation_attachment_retains_work_scope(&persisted) {
        return;
    }
...
async fn retire_work_scope_after_hard_delete(state: &AppState, deleted: &crate::db::Conversation) {
    ...
    let Some(scope_id) = deleted.attached_work_scope_id.clone() else {
        return;
    };
...
pub(crate) mod hard_delete_cascade_tests {
    ...
    async fn work_scope_inventory_empty_scope_returns_empty_inventory() {
        ...
        let conv = state
            ...
            .expect("conversation");
        let scope =
            crate::work_scope::ResourceScopeKey::Work(conv.attached_work_scope_id.expect("scope"));
    ...
    async fn create_inspector_actor(
        ...
        crate::work_scope::ResourceScopeKey::Work(
            actor.attached_work_scope_id.expect("actor work scope"),
        )
    }
