use super::types::{
    ...
    PrFeedbackSource, PrFeedbackStatus, PrFeedbackSummary, PrIdentity, PrRefreshMetadata,
    PrRefreshState, PrStatusResponse, PrUnavailableReason,
};
use crate::db::{
...
impl GhClient for ShellGhClient<'_> {
    ...
    fn reaction_status(
        ...
        let parsed: GhRootReactionResponse =
            serde_json::from_value(self.run_json_owned(&args, "root pull request reactions")?)
                .map_err(|e| GhFailure {
                    kind: GhFailureKind::CommandFailed,
                    message: format!("failed to parse root pull request reactions: {e}"),
                })?;
        let groups = parsed
            .data
            .and_then(|data| data.repository)
            .and_then(|repository| repository.pull_request)
            .map(|pull_request| pull_request.reaction_groups)
            .unwrap_or_default();
        Ok(status_from_reaction_groups(&groups))
    }
...
struct CapturedPrAutoFixContext {
    ...
    feedback_status: Option<PrFeedbackStatus>,
}
...
fn feedback_fingerprint(item: &PrFeedbackItem) -> String {
    feedback_content_fingerprint(item)
}

fn feedback_content_fingerprint(item: &PrFeedbackItem) -> String {
    format!(
        "{:?}:{}:{}:{}:{}:{}|{}",
        item.source,
...
fn status_from_reaction_groups(groups: &[GhReactionGroup]) -> PrFeedbackStatus {
    if groups
        .iter()
        .any(|group| group.content == "THUMBS_UP" && group.reactors.total_count > 0)
    {
        PrFeedbackStatus::Approved
    } else if groups
        .iter()
        .any(|group| group.content == "EYES" && group.reactors.total_count > 0)
    {
...
struct GhRootReactionResponse {
    data: Option<GhRootReactionData>,
}
#[derive(Debug, Deserialize)]
struct GhRootReactionData {
    repository: Option<GhRootReactionRepository>,
}
#[derive(Debug, Deserialize)]
struct GhRootReactionRepository {
    #[serde(rename = "pullRequest")]
    pull_request: Option<GhRootReactionPullRequest>,
}
#[derive(Debug, Deserialize)]
struct GhRootReactionPullRequest {
...
mod tests {
    ...
    fn feedback_summary(items: Vec<PrFeedbackItem>) -> PrFeedbackSummary {
        ...
        PrFeedbackSummary {
            ...
            unresolved,
            feedback_status: Some(PrFeedbackStatus::Open),
    ...
    #[test]
    fn root_reaction_groups_define_status_with_approval_precedence() {
        let eyes = GhReactionGroup {
            content: "EYES".to_string(),
            reactors: GhReactionGroupReactors { total_count: 1 },
        };
        let thumbs = GhReactionGroup {
            content: "THUMBS_UP".to_string(),
            reactors: GhReactionGroupReactors { total_count: 1 },
        };

        assert_eq!(
            status_from_reaction_groups(std::slice::from_ref(&eyes)),
            PrFeedbackStatus::InProgress
        );
        assert_eq!(
            status_from_reaction_groups(&[eyes, thumbs]),
            PrFeedbackStatus::Approved
        );
        assert_eq!(status_from_reaction_groups(&[]), PrFeedbackStatus::Open);
    }

    #[test]
    fn feedback_status_comes_from_root_reactions_not_comment_surfaces() {
        let gh = FakeGh {
            repo: Ok(repo()),
            issue_comments: Ok(vec![GhIssueComment {
                id: Some(1),
                ...
                body: Some("comment remains actionable".to_string()),
                html_url: Some("https://c/1".to_string()),
                created_at: Some("t".to_string()),
            }]),
            review_threads: Ok(vec![review_thread(
