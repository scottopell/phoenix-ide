    ...
    T: ToolExecutor + 'static,
{
    ...
    async fn process_acknowledged_event(
        ...
        if !matches!(&event, Event::SteeringQueueChanged) {
            ...
            if direct_turn {
                if !matches!(self.state, ConvState::Idle | ConvState::Error { .. }) {
                    return Ok(AcknowledgedEventOutcome::DirectTurnTemporarilyInadmissible);
                }
...
mod authoritative_user_message_effect_tests {
    ...
    async fn busy_authoritative_event_is_retryable_without_sse_or_state_publication() {
        ...
        let sequence_before = rt.broadcast_tx.current_seq();
        ...
        assert_eq!(
            second,
            AcknowledgedEventOutcome::DirectTurnTemporarilyInadmissible
        );
        assert_no_broadcast(&mut rx);
        assert_eq!(rt.broadcast_tx.current_seq(), sequence_before);
        assert!(!watch_rx.has_changed().unwrap());
