    ...
    match evidence {
        ...
        WakeTerminalEvidence::Subagent(value) => {
            let outcome = match &value.outcome {
                ...
                phoenix_workflow::wake_profile::SubagentTerminalOutcome::SubmitError { kind, error } =>
                    json!({"type":"submit_error","kind":kind,"error":error}),
                ...
                phoenix_workflow::wake_profile::SubagentTerminalOutcome::RuntimeFailure { kind } =>
                    json!({"type":"runtime_failure","kind":kind}),
                phoenix_workflow::wake_profile::SubagentTerminalOutcome::ContextExhausted =>
                    json!({"type":"context_exhausted"}),
                phoenix_workflow::wake_profile::SubagentTerminalOutcome::WallClockTimeout =>
                    json!({"type":"wall_clock_timeout"}),
                phoenix_workflow::wake_profile::SubagentTerminalOutcome::IndependentlyObservedCancellation =>
                    json!({"type":"independently_observed_cancellation"}),
                phoenix_workflow::wake_profile::SubagentTerminalOutcome::TurnLimitHardStop =>
                    json!({"type":"turn_limit_hard_stop"}),
            };
            json!({"type":"subagent","identity":resource_json(&WakeResourceIdentity::Subagent(value.identity.clone())),"occurred_at":value.occurred_at.0,"persisted_child_terminal_record":value.persisted_child_terminal_record.as_str(),"outcome":outcome})
        }
    }
}
