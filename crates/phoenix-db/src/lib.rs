...
impl Database {
    ...
    /// [`Self::is_auth_session_valid`] enforces.
    ///
    /// # Errors
    ///
    /// Returns a [`DbError`] if the underlying database operation fails.
    pub async fn insert_auth_session(&self, token: &str, ttl: chrono::Duration) -> DbResult<()> {
        let now = Utc::now();
    ...
    /// Whether `token` names a session that exists and has not expired.
    ///
    ...
    ///
    /// Returns a [`DbError`] if the underlying database operation fails.
    pub async fn is_auth_session_valid(&self, token: &str) -> DbResult<bool> {
        let now = Utc::now().to_rfc3339();
        let row: Option<(i64,)> =
            sqlx::query_as("SELECT 1 FROM auth_sessions WHERE token = ?1 AND expires_at > ?2")
                .bind(token)
                .bind(now)
                .fetch_optional(&self.pool)
                .await?;
...
mod tests {
    ...
    async fn auth_session_is_valid_after_insert_and_unknown_tokens_are_not() {
        let db = Database::open_in_memory().await.unwrap();
        db.insert_auth_session("tok-a", chrono::Duration::hours(1))
            .await
            .unwrap();
        assert!(db.is_auth_session_valid("tok-a").await.unwrap());
        assert!(!db.is_auth_session_valid("never-minted").await.unwrap());
    }
    ...
    async fn auth_session_expiry_is_enforced_and_swept() {
        ...
        db.insert_auth_session("stale", chrono::Duration::seconds(-1))
            .await
            .unwrap();
        assert!(!db.is_auth_session_valid("stale").await.unwrap());

        // A live token alongside it survives the sweep; the expired one is gone.
        db.insert_auth_session("fresh", chrono::Duration::hours(1))
            .await
            .unwrap();
        db.delete_expired_auth_sessions().await.unwrap();
        assert!(db.is_auth_session_valid("fresh").await.unwrap());

        // The expired row is physically gone: re-inserting the same token must
        // not collide on the primary key.
        db.insert_auth_session("stale", chrono::Duration::hours(1))
            .await
            .unwrap();
        assert!(db.is_auth_session_valid("stale").await.unwrap());
    }
