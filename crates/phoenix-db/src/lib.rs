...
impl Database {
    ...
    /// Atomically persist a direct user message and claim its conversation turn.
    ///
    /// Returns `Ok(None)` when the persisted conversation state no longer equals
    /// `expected_state`; the transaction is rolled back in that case.
    ///
    /// # Errors
    ///
    /// Returns a database error if serialization or any transactional statement
    /// fails.
    #[allow(clippy::too_many_arguments)]
    pub async fn claim_direct_user_turn(
        ...
        let expected_state_json = serde_json::to_string(expected_state)
            .map_err(|error| DbError::Serialization(error.to_string()))?;
        let claimed_state_json = serde_json::to_string(claimed_state)
            .map_err(|error| DbError::Serialization(error.to_string()))?;
        let mut tx = self.pool.begin().await?;
        ...
        if let Err(error) = retrieval::fts_upsert(&self.pool, &message).await {
            tracing::warn!(message_id, %error, "failed to index claimed direct message");
        }
        Ok(Some(message))
    }
