...
use phoenix_workflow::{
    ...
    Generation, LeaseExpiry, ManualChoiceKind, ManualResolutionId, ObservationId,
    ProcessIncarnation, ProfileRef, ReceiptFamily, ReceiptId, ReceiptOrigin, ResolutionStatus,
    RuntimeAcceptanceStatus, ScheduleId, ScheduleOccurrence, ScheduleOccurrenceId, SchedulePolicy,
    ScheduleStatus, SupportedCodecRegistry, SuppressionReason, Timestamp, TransitionId, Version,
    WorkflowBinding, WorkflowId, WorkflowStatus,
};
...
impl<'a> WorkflowTx<'a> {
    ...
    pub(crate) async fn adopt_receipt_and_delivery(
        &mut self,
        input: &AdoptReceiptInput,
    ) -> DbResult<ReceiptAcceptanceResult> {
        let effect = sqlx::query(
            "SELECT e.status, e.declared_workflow_version, e.generation,
                    a.attempt_id, a.process_incarnation,
                    EXISTS (
                        SELECT 1 FROM workflow_supported_codecs c
                        WHERE c.workflow_id = e.workflow_id
                          AND c.codec_family = ?6 AND c.codec_version = ?7
                    ) AS supports_receipt_codec,
                    EXISTS (
                        SELECT 1 FROM workflow_supported_codecs c
                        WHERE c.workflow_id = e.workflow_id
                          AND c.codec_family = ?8 AND c.codec_version = ?9
                    ) AS supports_receipt_event_codec
             FROM workflow_effects e
             JOIN workflow_authoritative_observations o
               ON o.workflow_id = e.workflow_id AND o.effect_id = e.effect_id
              AND o.observation_id = ?3
              AND o.declared_workflow_version = ?4 AND o.generation = ?5
             JOIN workflow_attempts a
               ON a.workflow_id = o.workflow_id AND a.attempt_id = o.attempt_id
              AND a.effect_id = o.effect_id
             WHERE e.workflow_id = ?1 AND e.effect_id = ?2
               AND a.status IN (
        ...
        let effect_status = effect.get::<String, _>("status");
        if effect.get::<i64, _>("declared_workflow_version")
            != i64::try_from(input.declared_workflow_version.0).unwrap()
            || effect.get::<i64, _>("generation") != i64::try_from(input.generation.0).unwrap()
            || !matches!(
                effect_status.as_str(),
                "Executing" | "Eligible" | "Receipted"
            )
        ...
        let attempt_id = AttemptId(to_u64(effect.get::<i64, _>("attempt_id"), "attempt_id")?);
        let process_incarnation = ProcessIncarnation(to_u64(
            effect.get::<i64, _>("process_incarnation"),
            "process_incarnation",
        )?);

        if let Some(existing) = sqlx::query(
            "SELECT r.receipt_id, r.effect_id, r.generation, r.declared_workflow_version,
                    r.process_incarnation, r.attempt_id, r.origin,
                    r.receipt_codec_family, r.receipt_codec_version, r.receipt_payload,
                    d.delivery_id, d.effect_id AS delivery_effect_id, d.barrier_id, d.consumer_kind,
        ...
        .await?
        {
            let receipt = LocalReceiptRecord {
                receipt_id: ReceiptId(to_u64(existing.get::<i64, _>("receipt_id"), "receipt_id")?),
                workflow_id: input.workflow_id,
                effect_id: EffectId(to_u64(existing.get::<i64, _>("effect_id"), "effect_id")?),
                generation: Generation(to_u64(existing.get::<i64, _>("generation"), "generation")?),
                declared_workflow_version: Version(to_u64(
                    existing.get::<i64, _>("declared_workflow_version"),
                    "declared_workflow_version",
                )?),
                process_incarnation: ProcessIncarnation(to_u64(
                    existing.get::<i64, _>("process_incarnation"),
                    "process_incarnation",
                )?),
                attempt_id: existing
                    ...
                    .transpose()?,
                origin: parse_receipt_origin(&existing.get::<String, _>("origin"))?,
                receipt_codec: local_codec(
                    existing.get("receipt_codec_family"),
                    existing.get("receipt_codec_version"),
                    "receipt_codec_version",
                )?,
                receipt_payload: existing.get("receipt_payload"),
            };
            let delivery = LocalDeliveryRecord {
                delivery_id: DeliveryId(to_u64(
                    existing.get::<i64, _>("delivery_id"),
                    "delivery_id",
                )?),
                workflow_id: input.workflow_id,
                effect_id: existing
                    .get::<Option<i64>, _>("delivery_effect_id")
                    .map(|v| to_u64(v, "delivery_effect_id").map(EffectId))
                    .transpose()?,
                barrier_id: existing
                    .get::<Option<i64>, _>("barrier_id")
                    .map(|v| to_u64(v, "barrier_id").map(BarrierId))
                    .transpose()?,
                consumer_kind: existing.get("consumer_kind"),
                event_codec: local_codec(
                    existing.get("event_codec_family"),
                    existing.get("event_codec_version"),
                    "event_codec_version",
                )?,
                payload_kind: parse_delivery_payload_kind(
                    &existing.get::<String, _>("payload_kind"),
                )?,
                payload_blob: existing.get("payload_blob"),
                requires_runtime_acceptance: existing.get("requires_runtime_acceptance"),
                status: parse_delivery_status(&existing.get::<String, _>("status"))?,
                runtime_acceptance_status: parse_runtime_acceptance_status(
                    existing.get::<Option<String>, _>("runtime_acceptance_status"),
                )?,
                suppression_reason: parse_suppression_reason(
                    existing.get::<Option<String>, _>("suppression_reason"),
                )?,
                accepted_by_transition_id: existing
                    .get::<Option<i64>, _>("accepted_by_transition_id")
                    .map(|v| to_u64(v, "accepted_by_transition_id").map(TransitionId))
                    .transpose()?,
            };
            let exact = receipt.effect_id == input.effect_id
                && receipt.generation == input.generation
                && receipt.declared_workflow_version == input.declared_workflow_version
                && receipt.process_incarnation == process_incarnation
                && receipt.attempt_id.is_none()
                && receipt.origin == ReceiptOrigin::Adoption
                && receipt.receipt_codec == input.receipt_codec
                && receipt.receipt_payload == input.receipt_payload
                && delivery.effect_id == Some(input.effect_id)
                && delivery.barrier_id.is_none()
                && delivery.consumer_kind == "reducer"
                && delivery.event_codec == input.receipt_event_codec
            ...
            return Ok(ReceiptAcceptanceResult {
                ...
                receipt: exact.then_some(receipt),
                delivery: exact.then_some(delivery),
            });
        }

        let effect_updated = sqlx::query(
            "UPDATE workflow_effects
             SET status = 'Receipted', pending_reconciliation = 0
             WHERE workflow_id = ?1 AND effect_id = ?2
               AND declared_workflow_version = ?3 AND generation = ?4
               AND status IN ('Executing', 'Eligible')",
        )
        .bind(to_i64(input.workflow_id.0, "workflow_id")?)
        .bind(to_i64(input.effect_id.0, "effect_id")?)
        .bind(to_i64(
        ...
        sqlx::query("INSERT INTO workflow_receipts (workflow_id, receipt_id, effect_id, generation, declared_workflow_version, process_incarnation, attempt_id, origin, receipt_codec_family, receipt_codec_version, receipt_payload) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, 'Adoption', ?8, ?9, ?10)")
            .bind(to_i64(input.workflow_id.0, "workflow_id")?)
            .bind(to_i64(input.receipt_id.0, "receipt_id")?)
            .bind(to_i64(input.effect_id.0, "effect_id")?)
            .bind(to_i64(input.generation.0, "generation")?)
            .bind(to_i64(input.declared_workflow_version.0, "declared_workflow_version")?)
        ...
        sqlx::query("INSERT INTO workflow_deliveries (workflow_id, delivery_id, effect_id, barrier_id, consumer_kind, event_codec_family, event_codec_version, payload_kind, payload_blob, requires_runtime_acceptance, status, runtime_acceptance_status, suppression_reason, accepted_by_transition_id) VALUES (?1, ?2, ?3, NULL, 'reducer', ?4, ?5, 'Receipt', ?6, ?7, 'Pending', ?8, NULL, NULL)")
            ...
            .bind(input.receipt_event_requires_runtime_acceptance)
        ...
        Ok(ReceiptAcceptanceResult {
            ...
            receipt: Some(LocalReceiptRecord {
                ...
                declared_workflow_version: input.declared_workflow_version,
                process_incarnation,
                attempt_id: None,
                origin: ReceiptOrigin::Adoption,
            ...
            delivery: Some(LocalDeliveryRecord {
                ...
                workflow_id: input.workflow_id,
                effect_id: Some(input.effect_id),
                barrier_id: None,
                ...
                requires_runtime_acceptance: input.receipt_event_requires_runtime_acceptance,
                status: DeliveryStatus::Pending,
                runtime_acceptance_status: input
                    .receipt_event_requires_runtime_acceptance
                    .then_some(RuntimeAcceptanceStatus::Owed),
...
impl WorkflowRepository {
    ...
    async fn adopt_receipt_once(
        ...
        let mut tx = self.begin_tx().await?;
        let result = tx.adopt_receipt_and_delivery(input).await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn adopt_receipt_after_restart_uses_authoritative_observation_without_new_attempt() {
        ...
        let recorded = repo
            .record_observation(&RecordObservationInput {
                ...
                observation_payload: vec![7],
            })
            .await
            .unwrap();
        assert_eq!(recorded.outcome, AuthorityOutcome::Authorized);

        let expired = repo
            .expire_lease_exact(&ExpireLeaseInput {
                workflow_id: WorkflowId(324),
                effect_id: EffectId(1),
                attempt_id: AttemptId(1),
                now: Timestamp(10),
        ...
        let adopted = repo
            ...
            .await
            .unwrap();
        assert_eq!(adopted.outcome, AuthorityOutcome::Authorized);
        assert_eq!(
            adopted.receipt.as_ref().unwrap().origin,
            ReceiptOrigin::Adoption
        );
        ...
        let claimed_again = second
            ...
            .await
            .unwrap();
        assert_eq!(claimed_again.outcome, ClaimOutcome::Ineligible);
        assert_eq!(repo.list_receipts(WorkflowId(324)).await.unwrap().len(), 1);
        assert_eq!(
            repo.list_deliveries(WorkflowId(324)).await.unwrap().len(),
            1
        );
    }
