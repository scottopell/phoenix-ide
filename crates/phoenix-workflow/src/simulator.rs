...
use crate::{
    AttemptId, BarrierId, CancellationRequest, ClaimAuthority, EffectId, EffectStatus, LeaseExpiry,
...
impl<P: WorkflowProfile> Simulator<P> {
    ...
    pub fn apply(&mut self, op: SimOp<P>) {
        match op {
            ...
            SimOp::CrashClaim { effect_id } => {
                if let Some(effect) = self.workflow.effects.get_mut(&effect_id) {
