...
The packaging phase copies the executable to `Contents/Helpers/phoenix_ide`, removes any stale helper during Debug no-sidecar builds, checks the packaged helper's architectures and exact repository-derived Phoenix build identity, and verifies nested signing when code signing is enabled. The app and sidecar form one release artifact; managed Phoenix's deployment updater does not own this sidecar.
