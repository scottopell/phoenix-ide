...
1. **Hold PR #471.** Do not merge the wake plane as a permanent bespoke scheduler.
2. **Use the rebased #471 branch as the integration line.** It is based on current `origin/main` and contains both durable creation and wake implementations.
3. **Wake is the first engine adopter.** PR #471 may merge after the engine-backed wake acceptance gate; creation shadow/cutover follows.
...
Write new ADRs rather than rewriting ADR-007 through ADR-009. Align wake, creation, and bedrock specs. Run the full spec-authoring preflight.
...
## Milestone 4 — migrate wake and merge PR #471
...
8. Merge PR #471 only after engine-backed wake passes the gate below.
...
- No merge of PR #471 before the engine-backed wake gate passes.
...
Milestone 1 normative artifacts are `specs/durable-workflows/requirements.md`, its executive summary, and ADR-010 through ADR-012. No Durable Workflows Allium artifact is part of this milestone.
