...
|---|---|---|---|---|
| G0 — readiness eligibility | #633 and task 55006/PR #677 are merged; retained R1 readiness evidence reports a clean query-only validation of its exact Project-ID partition, per-row ownership, attachment, observation-copy/absence, and strict-dormancy rules | sole live authority | dormant shadow only | The retained R1 result authorizes entry into the fenced cutover; it does not claim that its earlier live-DB snapshot remains current while Project writes continue. Abort on any reported mismatch, duplicate contradiction, partial source shape, live-reference leak, schema/version mismatch, or unmet R2 activation prerequisite. R1-valid absence is not itself a mismatch. No repair dual writes or live R1 probes. |
...
R1 readiness evidence is a durable eligibility input, not a lease on a live database and not proof that its earlier snapshot remains current. R2 consumes the exact R1 validator result named by the merged R1 authority, then quiesces and fences every writer before establishing its final SQLite-consistent target state. R2 must revalidate the exact R1 partition, attachments, observation copy-or-absence rules, and strict dormancy and complete its own identity resolution against that target state immediately before G3, with no intervening mutation.

The durable R2 cutover evidence must bind the consumed R1 readiness result and final validation result to the exact cutover binary identity, schema identity, and target authority generation. These are semantic bindings; this task does not prescribe a dump/copy/WAL command, artifact path, CI vendor, or storage mechanism.
...
- [ ] The retained R1 readiness result is consumed as eligibility evidence only; after quiescence/fencing, final query-only validation on one SQLite-consistent target state proves Project-ID partition/cardinality, exact owned-field equality, WorkScope attachment equality, observation copy-or-absence rules, duplicate-contradiction failure, and strict dormancy immediately before G3 with no intervening mutation. R1-valid absence remains absence until R2 resolves or types it without fallback.
- [ ] Durable cutover evidence binds the consumed R1 readiness result and final validator/result to the exact cutover binary identity, schema identity, and target authority generation without depending on a particular artifact path, CI system, or database-copy mechanism.
