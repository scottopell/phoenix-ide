...
- Readiness-handoff head at this review: [`0052796a847e304d956c212de88695651456fbf0`](https://github.com/scottopell/phoenix-ide/commit/0052796a847e304d956c212de88695651456fbf0)
...
- R1 task 55006/PR #677 merged with its canonical non-composable readiness evidence bundle complete and clean: additive hidden `GitRepository` substrate, deterministic replay-stable Project-ID-partitioned backfill, dormant singular nullable `WorkScope.repository` attachment, exact implementation/schema and target validator snapshot/run, authority census, old-binary compatibility result, and rollback posture. Current readiness-handoff planning head: `0052796a847e304d956c212de88695651456fbf0`.
...
| G3 — final target-state validation and DB authority transaction | After G1 quiescence/fencing, establish one SQLite-consistent target snapshot/state; revalidate the exact R1 partition and strict dormancy and resolve R2 identity against that same state immediately before the flip; with all writers still fenced and no intervening mutation, record the irreversible authority generation, disable legacy Project mutations, and activate hidden-authority constraints transactionally | retained read-only data/projection only | enabled | Abort on any final R1 or R2 mismatch. The authority transaction rolls back wholly on failure. Durable cutover evidence binds the consumed R1 readiness result, final validator/result, exact binary identity, schema identity, and target authority generation without prescribing storage mechanism. The DB transaction does not pretend to atomically include binary files or service-manager handoff; durable backend deployment claims bracket G1–G4. No request/background admission until commit and exact-binary activation are both proven. |
...
The canonical R1 readiness evidence bundle defined by merged task 55006 is a durable, non-composable eligibility input, not a lease on a live database and not proof that its earlier snapshot remains current. R2 consumes the whole bundle—never members mixed across implementation heads, schema states, target snapshots, validator runs, compatibility exercises, or rollback postures—then quiesces and fences every writer before establishing its final SQLite-consistent target state. R2 must revalidate the exact R1 partition, attachments, observation copy-or-absence rules, and strict dormancy and complete its own identity resolution against that target state immediately before G3, with no intervening mutation.

The durable R2 cutover evidence must bind the consumed R1 bundle and final validation result to the exact cutover binary identity, schema identity, and target authority generation. These are semantic bindings; this task does not prescribe a dump/copy/WAL command, artifact path, CI vendor, or storage mechanism.
...
- [ ] One complete canonical R1 readiness evidence bundle is consumed as eligibility evidence only; its implementation/schema, target validator snapshot/run, typed result, authority census, old-binary compatibility result, and rollback posture are not mixed across runs. After quiescence/fencing, final query-only validation on one SQLite-consistent target state proves Project-ID partition/cardinality, exact owned-field equality, WorkScope attachment equality, observation copy-or-absence rules, duplicate-contradiction failure, and strict dormancy immediately before G3 with no intervening mutation. R1-valid absence remains absence until R2 resolves or types it without fallback.
