...
PR #635 exposed four correctness bugs that are independently reachable on `main`; they do not require parallel Work subagents.
...
4. **Pre-install cancellation does not revoke admission:** cancellation consumed while a spawn is queued or materializing can synthesize a cancelled parent result before any runtime exists, but the queued spawn request remains live and can later create and run an orphan child after the parent has settled.
...
- Give cancellation one child-identity authority across queued, materializing, and installed phases: cancellation before dequeue revokes admission, cancellation during materialization is owed at installation, and installed-child cancellation remains prompt without waiting behind unrelated materialization.
...
- Cancellation before dequeue prevents runtime creation and tool execution while producing exactly one parent result.
- Cancellation during materialization is delivered exactly once after installation, before child work begins.
