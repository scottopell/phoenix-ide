...
### browser_type
- [x] ~~Support special keys beyond Enter (Tab, Escape, Arrow keys)~~ — done via `browser_key_press`
- [ ] Add `delay` option for realistic typing speed
- [x] ~~Support key combinations (Ctrl+A, Cmd+V)~~ — done via `browser_key_press`
...
- [x] ~~`browser_key_press`~~ — **implemented** (commit 2a74bb4); covers keyboard shortcuts and non-printable keys (Tab, Escape, Arrow keys, Ctrl+A, Cmd+V, etc.)
...
- [ ] `browser_inject_react_devtools` - Access React component tree/state — see task NNN
