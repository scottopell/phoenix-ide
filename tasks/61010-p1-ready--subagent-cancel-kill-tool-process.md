...
4. Update specs/bash (`bash.allium` / `requirements.md`): cancel still never kills for the PARENT/normal case; sub-agent TEARDOWN forces a kill of that sub-agent's own handles. Keep the parent detach-dont-kill invariant; carve out sub-agent teardown.
