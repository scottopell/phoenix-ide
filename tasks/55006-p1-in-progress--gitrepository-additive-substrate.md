...
R1 produces one canonical **R1 readiness evidence bundle** for each candidate implementation and validator run. The implementation represents it as one immutable typed run identity carried by every member and verified by the R2 consumer, so members from different implementation heads, schema states, process-local database lifecycles, query-only snapshots, or validator runs cannot form a valid bundle. A later run mints a new identity and supersedes an earlier bundle as a whole.

The bundle binds:

- the exact R1 implementation identity and expected schema/migration identity;
- the process-local target database lifecycle, one validator run identity, and the single query-only SQLite transaction/snapshot boundary from which database results were read;
- the validator's typed overall result plus every category and bounded count, distinguishing valid absence from mismatch or incomplete/conflicting evidence; and
- the repository-authority census revision for that implementation, including the conclusion that Project remains the sole live authority and no R1 production reader, writer, fallback, repair, or dual-write path exists.

A bundle is complete only when those members come from the same implementation/schema and readiness run, validation is clean, and the census has no authority leak. Valid absence remains typed/countable evidence. The bundle performs no live probe, repair, synchronization, authority selection, rollback, or cutover and prescribes no serialization or storage product.
...
- [ ] Catch-up boundary tests prove invocation is structurally impossible without a one-shot typed capability minted only after successful exclusion of every legacy writer; failed/incomplete exclusion mints none; database/exclusion-operation mismatch and reuse are rejected; final catch-up remains unconditional once authorized; every post-backfill Project/WorkScope write invalidates the old bundle; deterministic transactional reconciliation inserts missing, replaces superseded, and removes source-less dormant rows without mutating legacy source; intrinsic source/identity conflicts fail without partial commit; and validation issues a wholly new bundle identity.
...
- [ ] One complete R1 readiness evidence bundle uses an immutable typed run identity verified on consumption and binds the exact implementation/schema, process-local database lifecycle, validator snapshot/run, typed result/categories/counts, and production authority census without mixing evidence across runs.
...
| R1→R2 handoff ambiguity | Individually valid validator and census evidence could come from different heads, schema states, database snapshots, or runs. | Requires one immutable typed run identity carried by every member and verified by R2, tied to one implementation/schema, process-local database lifecycle, and validator transaction/run; valid absence remains explicit, and R2 still owns post-quiescence revalidation and activation. |
