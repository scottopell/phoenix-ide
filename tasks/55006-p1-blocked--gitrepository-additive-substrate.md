...
3a. after any interval that admitted legacy Project or WorkScope writes since the last successful backfill—whether from the R1 binary or a rolled-back #633-era binary—quiesce and exclude those writers, then run the deterministic backfill again as one idempotent transactional catch-up before readiness validation; this is not a steady-state writer;
...
Any legacy Project or WorkScope write admitted after the database boundary named by a readiness bundle invalidates that bundle, regardless of which pre-cutover binary performed the write. Before a replacement bundle can be minted, every such writer is quiesced and excluded; the catch-up then replays the same total Project-ID mapping, exact WorkScope-assignment rules, and retained-observation selection over the then-current Project authority, inserts missing dormant rows/attachments/observations, verifies existing seeded rows rather than overwriting conflicts, and commits wholly or not at all. Validation runs only after that transaction and mints a new readiness bundle. No catch-up runs concurrently with a legacy writer, no normal request path invokes it, and it never reads Git/filesystem/network state or promotes hidden data to live authority.

R1 provides this idempotent catch-up capability and its tests. R2 owns the final sequence that quiesces/excludes legacy writers, invokes catch-up when any post-bundle legacy write exists, reruns query-only validation on the resulting SQLite-consistent state, and decides whether to activate. R1 neither initiates that final sequence nor treats a pre-quiescence bundle as activation authority.
...
- [ ] Catch-up tests prove every post-backfill Project/WorkScope write—ordinary R1-era or rollback-era—invalidates the old bundle; deterministic transactional catch-up fills only missing dormant rows/attachments/observations after all legacy-writer exclusion; conflicts fail without partial commit; and validation issues a wholly new bundle identity.
...
| Post-backfill legacy-write gap | Project/WorkScope rows created by either the R1 binary or a rolled-back binary after the latest backfill would lack dormant rows when an applied migration does not rerun. | Every post-bundle legacy write invalidates readiness; after all legacy writers are quiesced/excluded, the idempotent backfill runs as one transactional catch-up, validation compares exact source-derived state, and a new whole-bundle identity is required. R2 owns the final quiesce/catch-up/revalidate/activate sequence. |
