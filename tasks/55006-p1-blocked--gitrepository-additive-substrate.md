...
3a. after any rollback interval that admitted #633-era Project writes, exclude that old binary and run the deterministic backfill again as one idempotent transactional deployment migration/catch-up before validation; this is not a steady-state writer;
...
Any Project write admitted while the preceding binary is active invalidates the earlier readiness bundle. On forward redeploy, the preceding binary is excluded before the catch-up transaction; the catch-up replays the same total Project-ID mapping and exact WorkScope-assignment rules over the then-current Project authority, inserts missing dormant rows/attachments, verifies existing seeded rows rather than overwriting conflicts, and commits wholly or not at all. Validation runs only after that transaction and mints a new readiness bundle. No catch-up runs concurrently with an old writer, no normal request path invokes it, and it never reads Git/filesystem/network state or promotes hidden data to live authority.
...
- [ ] Rollback/forward tests prove Project writes invalidate the old bundle, deterministic transactional catch-up fills only missing dormant rows/attachments after old-writer exclusion, conflicts fail without partial commit, and validation issues a wholly new bundle identity.
...
| Rollback-era write gap | Project/WorkScope rows created after the one-time upgrade would lack shadow rows when an applied migration does not rerun. | Any rollback-era write invalidates readiness; forward deployment excludes old writers, reruns the idempotent backfill as one transactional catch-up, validates, and issues a new whole-bundle identity before R2 eligibility. |
