...
R1 provides this idempotent deterministic reconciliation capability and its tests. R2 owns the final sequence that quiesces/excludes legacy writers, invokes catch-up unconditionally, reruns query-only validation on the resulting SQLite-consistent state, and decides whether to activate. R1 neither initiates that final sequence nor treats a pre-quiescence bundle as activation authority.
...
- [ ] Catch-up tests prove final catch-up is unconditional after all legacy-writer exclusion; every post-backfill Project/WorkScope write—ordinary R1-era or rollback-era—invalidates the old bundle; deterministic transactional reconciliation inserts missing, replaces superseded, and removes source-less dormant rows without mutating legacy source; intrinsic source/identity conflicts fail without partial commit; and validation issues a wholly new bundle identity.
