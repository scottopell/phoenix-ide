...
R1 produces one canonical **R1 readiness evidence bundle** for each candidate R1 implementation and target database. The implementation represents it as one immutable typed bundle/run identity carried by every member and verified by the R2 consumer, so members from different candidate R1 implementation heads, candidate schema states, target database snapshots, or validator runs cannot form a valid bundle. This structural identity does not prescribe a serialization format or storage mechanism. The old-binary exercise intentionally names one different, pinned preceding binary as its test subject; its result must be produced against this bundle's candidate R1 expanded schema and cannot be borrowed from another candidate. A later run mints a new identity and supersedes an earlier bundle as a whole rather than allowing evidence to be mixed across runs.
...
3a. before the final readiness validation consumed by R2, quiesce and exclude every legacy Project/WorkScope writer, then run the deterministic backfill as one unconditional idempotent transactional catch-up—even when no post-backfill write is known—because R1 installs no steady-state write-generation signal; this is not a steady-state writer;
...
- [ ] One complete R1 readiness evidence bundle uses an immutable typed bundle/run identity verified on consumption and binds the exact implementation/schema, target database snapshot and validator transaction/run, typed result/categories/counts, production authority census, old-binary compatibility result, and rollback posture without mixing evidence across runs.
- [ ] Old-binary compatibility is exercised end to end: upgrade with R1; run the exact immediately preceding #633-era binary against the expanded database on representative Project/WorkScope read/write journeys; exclude that old binary; forward-redeploy the candidate R1 binary; invoke unconditional transactional catch-up; obtain a clean query-only validation result matching the post-write legacy source-derived state; and mint a fresh readiness bundle identity that supersedes the pre-write bundle as a whole. The exercise proves old writes remain supported and recoverable, not that pre-write counts remain unchanged.
...
| Old-binary exercise gap | Proving that an old binary can write the expanded schema does not prove forward recovery of the now-stale dormant shadow. | The exercise excludes the old writer, forward-redeploys R1, invokes unconditional transactional catch-up, validates the post-write source-derived state, and mints a fresh whole-bundle identity. |
...
| R1→R2 handoff ambiguity | Individually valid validator, census, compatibility, and rollback artifacts could come from different heads, schema states, database snapshots, or runs. | Requires one immutable typed bundle/run identity carried by every member and verified by R2, tied to an exact implementation/schema and one target validator transaction/run; valid absence remains explicit, and R2 still owns post-quiescence revalidation and activation. |
