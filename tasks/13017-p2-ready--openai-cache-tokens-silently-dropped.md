...
- crates/phoenix-ide/src/llm/types.rs:283-294 — shared `Usage` has all four fields and `total()` sums all four
...
## Fix direction
Add `cached_tokens` (and any cache-creation analogue the Responses API exposes) to `ResponsesApiUsage`, parse `usage.input_tokens_details.cached_tokens` in both the streaming accumulator (openai.rs ~339) and non-streaming `normalize` (openai.rs ~776), and map into `Usage.cache_read_tokens`. If the Responses API genuinely has no cache-creation concept, make that a typed/commented sink rather than a bare `0`, and emit a `tracing::debug!` where a capability gap is hit.
