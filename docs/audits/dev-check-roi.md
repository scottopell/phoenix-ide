...
| `clippy` | `cargo clippy --all-targets -- -D warnings`; static/pedantic defects including tests. An ordinary unfiltered check on crate-confined Rust changes reuses the changed-crate plus reverse-dependency `-p` scope; `--all`, unsafe-to-narrow inputs, and CI's isolated `--lanes clippy` invocation lint the full workspace. | **Full-workspace `--all` 60.8 s**; profile 88.6 s, **8.3%**. Ordinary scoped local cost was not measured. Cold: unknown. | 31/50 (62%) | Dedicated non-incremental `target/clippy`: stale natural build allocated **937,040 KiB (915 MiB)**; current refresh added 47,044 KiB. Isolation prevents clippy-driver invalidating Rust-test fingerprints; source/toolchain changes rebuild. | CI `check (clippy)` runs full workspace; local default may be package-scoped. It overlaps compilation but catches lint classes tests do not. | **Conditional (retain).** CI-only would save an unknown scoped cost on crate-confined local changes and up to the measured ~61 s full-workspace cost, but delay static feedback; the dedicated target trades ~915 MiB for avoiding cross-lane invalidation. |
