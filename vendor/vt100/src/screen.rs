...
impl Screen {
    fn text(&mut self, c: char) {
        ...
        // Guard: a zero-dimension terminal cannot safely draw any character.
        // All arithmetic below (size.cols - width, pos.col - 1, pos.col + 1)
        // assumes size.rows > 0 and size.cols > 0. Return early rather than
        // panic on degenerate sizes (e.g. cols=1 with a width-2 character).
        ...
        let width: u16 = width
            ...
            .unwrap();
        // Clamp width to the terminal columns so all subsequent arithmetic
        // (size.cols - width) cannot underflow on narrow terminals. Fixes
        // panics when wide chars (width=2) appear on a 1-column terminal.
        let size = self.grid().size();
        let width: u16 = width.min(size.cols.max(1));
        ...
        let mut wrap = false;
        if size.cols > 0 && width <= size.cols && pos.col > size.cols - width {
            let last_cell = self
                .grid()
        ...
        if width == 0 {
            ...
            // Use if-let throughout to avoid panics when grid state is
            // inconsistent (e.g. wide-char metadata references a col that
            // no longer exists after a resize or unusual input sequence).
            if self.grid().drawing_cell(pos).map_or(false, |c| c.is_wide_continuation()) {
            ...
            if self.grid().drawing_cell(pos).map_or(false, |c| c.is_wide()) {
