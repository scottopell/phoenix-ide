...
- The **StateBar** renders the **PR badge / PR identity link** (`StateBarPrBadge`) — the active
  PR's human-facing identity and status: number, title, state. A StateBar concern.
...
(REQ-PRA-001) lives next to the Address-CI action, not on the StateBar badge, and remains an
actionability signal rather than a second PR-status surface. A singular primary-PR view may exist
as a compatibility projection for singular consumers, but the authoritative owner for PR-specific
targeting is the explicit active PR.
...
| REQ-PRA-000b | PR-specific surfaces share one explicit target — StateBar identity/status stay aligned with work-actions freshness/auto-fix and sibling PR-specific surfaces without collapsing their distinct roles |
...
| REQ-PRA-000b | Implemented | `crates/phoenix-ide/src/api/git_handlers.rs` (`get_pr_status`, PR feedback capture, `get_active_pr_diff` all target `active_selection_target_for_scope`); `crates/phoenix-ide/src/address_feedback_workflow.rs` (durable Address feedback capture + handoff); `ui/src/components/WorkActions.test.tsx` (active PR interactions + diff targeting); `ui/src/components/StateBar.tsx` (active PR selector) |
