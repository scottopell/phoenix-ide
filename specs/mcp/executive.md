...
| REQ-MCP-017 | Tool Call Cancellation and Error Surfacing | Complete | `McpTool::run` passes cancellation into `McpClientManager`; cancellation before dispatch returns immediately, while cancellation during a stdio exchange exclusively removes and re-establishes that serving transport before returning. `tools/call` honors `isError`. |
