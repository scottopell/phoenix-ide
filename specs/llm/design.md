...
| `openai_responses` | `OPENAI_RESPONSES_BASE_URL` as-is, legacy `OPENAI_BASE_URL` when it is not a Chat Completions endpoint, or `https://api.openai.com/v1/responses` | OpenAI Responses |
| `openai_chat_completions` | `OPENAI_CHAT_COMPLETIONS_BASE_URL` as-is, legacy `OPENAI_BASE_URL` when it is not a Responses endpoint, or `https://api.openai.com/v1/chat/completions` | OpenAI Chat Completions |
...
The registry starts from built-in model specs and merges valid additive specs from `PHOENIX_LLM_MODELS`. External specs declare a backend and may omit `api_name`, in which case Phoenix sends `id` as the wire model name. External specs may provide `max_output_tokens`; explicit values must be nonzero and lower than `context_window`, and omitted values are capped below small context windows. Duplicate IDs keep the built-in or earlier configured definition.
...
| `https://proxy.example/v1/chat/completions` | `https://proxy.example/v1/chat/models` (not used for Responses filtering) |

Discovery is backend-scoped. Anthropic discovery results only validate `anthropic` models; OpenAI Responses discovery results only validate `openai_responses` models. Chat Completions models are additive configuration entries and do not share the Responses discovery bucket. If discovery returns no usable configured models, Phoenix falls back to the configured model list and logs a warning.
