...
The LLM provider abstracts communication with multiple LLM APIs behind a common interface. Users select their preferred model while the system handles provider-specific translation internally. When exe.dev gateway is configured, requests route through it with provider-specific path suffixes; otherwise direct API connections are used. The model registry enumerates available models based on configured API keys, hiding unavailable options. Requests use a common format (system prompt, messages, tools) that gets translated per-provider. Responses are normalized to text blocks, tool use requests, end-of-turn indicators, and usage statistics. Errors are classified as retryable (network, rate limit) or non-retryable (auth) to enable appropriate state machine handling.

## Technical Summary

Implements `LlmService` trait with `complete()` method returning `LlmResponse`. Provider implementations (Anthropic, OpenAI, Fireworks) translate common request format to provider-specific JSON and normalize responses back. Gateway URLs constructed by appending provider suffix to base gateway URL. `ModelRegistry` holds `HashMap<String, Arc<dyn LlmService>>` populated based on available API keys. `LlmError` includes `LlmErrorKind` enum with `is_retryable()` method. `LoggingService` wrapper records timing, token counts, and optionally raw request/response bodies for debugging. Usage tracking includes input/output tokens, cache statistics, and estimated cost.
