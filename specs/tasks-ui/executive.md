...
|---|---|---|
| **REQ-TASKS-UI-001:** See My Tasks Without Leaving the Conversation | ✅ Complete | `ui/src/components/TasksPanel.tsx:34-106` (collapsible header, list); `:50-64` (defer load until expanded) |
...
| **REQ-TASKS-UI-007:** Pay the Load Cost Only When I Look | ✅ Complete | `TasksPanel.tsx:50-64` — `useEffect` is gated on `expanded`, so closing the panel for the conversation lifetime never fetches |

**Progress:** 7 of 7 complete. Read-only UI, no spec gaps.
