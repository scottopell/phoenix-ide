...
contract fires only the registering conversation. Bash contracts
fire `Forgotten` on the continuation boundary because bash handles
themselves are conv-keyed and die at the boundary (per task 62009
direction B); tmux / browser / subagent contracts transfer their
`conv_id` to the child because those handles are WorkScope-keyed and
themselves transfer.
...
| REQ-WAKE-012 Continuation Inheritance | Proposed | Bash forgets; tmux/browser/subagent transfer |
...
- **Task 62009** ([[bash-cascade-skips-inheritor-scope]], p1) —
  direction resolved (B: document asymmetry + notify child);
  REQ-WAKE-012 conditioned on this
