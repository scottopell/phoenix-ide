...
| REQ-RU-003 | `client_is_local`, `valid_approval`, and controller exact-tag/full-commit validation enforce same-host, preview-bound approval; the deployment summary presents locality independently, release discovery changes only on initial/explicit checks, and active transaction polling stops on terminal status or unmount. |
