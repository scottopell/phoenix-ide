...
THE SYSTEM SHALL sign the nested server helper and outer app with the configured Developer ID identity,
SHALL apply the app's hardened-runtime entitlements,
...
THE SYSTEM SHALL provide an unsigned local-test mode that verifies app construction, embedded-helper identity, architecture, and archive naming without weakening the signed release path.
