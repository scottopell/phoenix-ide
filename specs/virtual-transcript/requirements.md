...
AND SHALL carry that owed correction as the position it must reach rather than a distance to travel, so that geometry resynchronized from the platform in the interval — including a viewport the platform itself clamps as the rendered block shortens — cannot re-base it and apply part of the shift twice
