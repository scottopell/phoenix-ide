...
4. Expose unrestricted production SQLite, including credentials and writes.
...
The SQL boundary is engine-enforced. A separate read-only connection installs a SQLite authorizer and progress handler. It denies writes, transactions, pragmas, attached databases, extensions, filesystem functions, SQLite internal/shadow storage, credential/session/token tables, and secret settings. Results have host-owned row, byte, and execution budgets.
...
- **Positive:** Credential and filesystem boundaries are structural rather than prompt-based.
- **Negative:** The Coordinator is coupled to allowed database schema and must formulate SQL.
