...
Conversation-local persisted message sequences are unique and strictly increasing for new inserts. Released databases with duplicate sequences are migrated by preserving every row, ordering each affected conversation by durable `(old sequence_id, created_at, message_id)`, deterministically stable-shifting affected sequence identities when required, permitting repaired identities to remain sparse rather than dense, remapping `clear_watermark` by stable cleared-row identity/order, bumping transcript generation for each affected conversation, then installing schema-level uniqueness and monotonic-insert enforcement.
