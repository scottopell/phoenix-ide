...
Conversation-local persisted message sequences are unique and strictly increasing for new inserts. Released databases with duplicate sequences are migrated by preserving every row, ordering each affected conversation by durable `(old sequence_id, created_at, message_id)`, densely renumbering that partition when required, remapping `clear_watermark` to the greatest repaired sequence whose original row was already cleared, bumping transcript generation for each affected conversation, then installing schema-level uniqueness and monotonic-insert enforcement.
