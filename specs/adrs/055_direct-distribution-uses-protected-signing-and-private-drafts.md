# ADR-055: Direct distribution uses protected signing and private draft publication
...
Publication creates or resumes only a draft for the exact tag. The protected publisher is the sole authorized writer for that release while it validates any existing draft assets as an exact digest-matching subset, uploads only missing assets, verifies the complete draft, rechecks tag identity, and only then makes the draft public. It never deletes an existing release during recovery. A public exact release is idempotent. A differing public release fails without mutation.
