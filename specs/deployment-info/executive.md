...
| **REQ-DEPLOY-002C:** Make diagnostic freshness and refresh scope explicit | Implemented | Deployment facts, visible-page resource observations, and disk inventory show independent freshness/sample times and scope-labeled refresh controls. Failed refreshes retain and label last-good snapshots as stale; absent snapshots are unavailable. |
...
| **REQ-DEPLOY-004E:** Report macOS thermal capability and governor status without fabrication | Specified | Typed thermal-pressure, raw-temperature-unavailable, and governor-mode semantics are now part of the normative deployment-resource contract, but this branch intentionally changes specs only. No implementation evidence is claimed here yet. |
| **REQ-DEPLOY-004F:** Observe-only macOS governor decisions remain non-mutating | Specified | Observe-only semantics are now normative: the governor samples real provider state, evaluates the same decision engine as enforcement, records proposed actions, and produces no native scheduler or lifecycle effects. This branch adds the requirement and behavioural model only. |
