...
| **REQ-DEPLOY-004D:** Keep deployment facts and live resource monitoring separate | Implemented | `GET /api/deployment` carries build, network, log, and locality facts without a parallel resource snapshot; `GET /api/about/resources` is the sole live CPU, memory, load, and managed-process telemetry contract; and `GET /api/deployment/disk` owns explicit disk sizing. The section resource refresh and page-level refresh both request a fresh visible resource sample; disk remains independently refreshable. |
