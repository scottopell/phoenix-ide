...
- **Auto-cleanup**: Browser closes on conversation end, or on the 30-minute idle timeout once the scope owns no live conversation (the timer is a backstop for abandoned scopes, not a hard cap on live work — see "Idle Timeout" below)
...
Subsequent tool calls reuse browser (updates last_activity timestamp)
    |
    v
[(30 min idle AND scope owns no non-terminal conversation) OR conversation deleted OR server shutdown]
    |
    v
...
- A scope is reaped only when it owns no live conversation — one that is both non-terminal in state and not `archived`: the manager consults an injected scope-liveness predicate (set by the runtime alongside the lifecycle bridges, via a `Weak` so it cannot keep the runtime alive) that derives liveness from the persisted conversation rows in the DATABASE — querying the conversations on the scope's worktree path and resolving each to a `WorkScope` — NOT from the set of live runtime handles (REQ-BROWSER-WS-002). A non-terminal conversation with no runtime handle (after a server restart or runtime eviction) keeps the scope live. A live scope is skipped and re-checked next interval; with no predicate wired (tool-level tests), reaping is age-only. This prevents force-closing Chrome under a conversation that is alive in the UI but has merely been quiet on browser tools
