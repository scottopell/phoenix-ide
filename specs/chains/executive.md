...
| **REQ-CHN-010:** Regenerate Chain Name From Member Content | ✅ Complete | Manual `POST /api/chains/:rootId/regenerate-name` (`regenerate_chain_name` in `api/chains.rs`) walks members, gathers each `first_opening_message_text` (user message or skill trigger), summarizes via `title_generator::generate_chain_name`, and persists the `chain_name` override through the shared `normalize_chain_name`; UI is the regenerate button in `ChainPage`'s header. Name left untouched on any failure |
