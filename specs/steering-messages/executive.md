...
- **P1 (enqueue):** `enqueue_steer_message` reads the current queue, appends the new entry, writes the updated queue to the DB, broadcasts a renderable `steer_message_queued` event, and then sends `Event::SteerMessage` to the executor channel. The HTTP response returns only after the DB write succeeds. A crash between acceptance and executor processing does not lose the entry.
- **P2 (cancel):** the cancel handler updates the DB, broadcasts `steer_message_cancelled`, then sends `Event::CancelSteerMessage` to the live executor (if running), which removes the matching entry from in-memory state without a further DB write.
...
- **`specs/sse_wire/`**: `init` carries the durable steering queue, while `steer_message_queued` and `steer_message_cancelled` carry live mutations. Persistence-before-broadcast applies. Drained entries surface through the normal `message` path and remove the matching queued identity client-side.
