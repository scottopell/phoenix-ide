...
- **P1 (enqueue):** `enqueue_steer_message` atomically appends one normalized queue row plus its immutable acceptance receipt and obtains the committed FIFO position, broadcasts a live-only `steer_message_queued` projection, and then sends `Event::SteerMessage` to the executor channel. The HTTP response returns only after the DB transaction succeeds. A failed append persists neither row and emits no success projection. Once append commits, executor delivery or immediate reconstruction failure cannot turn durable admission into a rejection. Phoenix makes one bounded reconstruction retry; if it also fails, the stale preserved broadcaster is released so connected clients reconnect through ordinary runtime construction while the queued row remains authoritative.
...
A committed drain in `LlmRequesting` remains restart-owned only while the latest transcript row is a user or skill message carrying an immutable steering acceptance receipt and that exact queue row is absent. Runtime reconstruction resumes one LLM request from this evidence; a synchronous failure to start it is reduced into a persisted typed error. If that error write fails, the executor retires so reconnect reconstructs from unchanged database truth. The first provider response changes the transcript tail and returns later interruptions to ordinary Bedrock restart policy; no steering-specific lifecycle row or second dispatch authority exists.
...
| **Committed steering restart recovery** | Complete | `Database::has_committed_steering_turn`, startup reset, and `RuntimeManager::determine_resume_state` preserve only a receipt-backed latest user or skill turn until its first response; startup dispatch failures settle through the typed error reducer or retire on failed persistence. |
