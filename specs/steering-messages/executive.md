...
| **Committed steering restart recovery** | Complete | `Database::has_committed_steering_turn`, startup reset, and `RuntimeManager::determine_resume_state` preserve only a receipt-backed latest user turn until its first response; startup dispatch failures settle through the typed error reducer or retire on failed persistence. |
