...
- **P2 (cancel):** under the same per-conversation admission gate used by sends, the cancel handler deletes the exact queue row and learns whether a row was removed. Only a successful removal broadcasts the live-only `steer_message_cancelled` projection. The idempotent executor removal is always sent when a runtime is live; cancellation also joins an already-running materialization without starting a new runtime, reconciling a queue snapshot loaded before the delete. Cancelling an absent row remains an idempotent 200 without publishing a false cancellation.
