...
- **P2 (cancel):** under the same per-conversation admission gate used by sends, the cancel handler deletes the exact queue row and learns whether a row was removed. Only a successful removal broadcasts the live-only `steer_message_cancelled` projection. Runtime startup and queue mutations also share a narrow projection fence: startup reloads the durable queue immediately before publishing and spawning the executor, so cancellation either commits before that snapshot or runs afterward and sends the idempotent in-memory removal. Cancelling an absent row remains an idempotent 200 without publishing a false cancellation.
...
New admissions use `Database::append_steering_entry`, which inserts one normalized entry and returns its committed position in one transaction; cancellation uses an exact-row delete. Bulk queue replacement remains available for lifecycle paths that own the complete queue value, while drain cleanup removes only the drained identities. On startup the executor is seeded via `with_steering_queue`, sourced from a final `Database::get_steering_queue` read under the same narrow projection fence used by queue mutations. That read rehydrates each entry's attachments and skill invocation from one durable snapshot before the executor is published or spawned. The queue is therefore live immediately after a Phoenix restart — no warm-up.
