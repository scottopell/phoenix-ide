...
New admissions use `Database::append_steering_entry`, which inserts one normalized entry and returns its committed position in one transaction; cancellation uses an exact-row delete. Bulk queue replacement remains available for lifecycle paths that own the complete queue value, while drain cleanup removes only the drained identities. On startup the executor is seeded via `with_steering_queue`, sourced from a final `Database::get_steering_queue` read under the same narrow projection fence used by queue mutations. Before each drain, `MessageStore::load_steering_entries` refreshes that projection under the fence, which remains held until the drained identities are persisted and removed. The queue is therefore live immediately after a Phoenix restart and cannot deliver a durably cancelled startup entry.
