...
| REQ-CCR-006 Runtime Bootstrap | Partial | Initial-message creation checkpoints `Finalize`, then atomically commits ready job status with the dispatchable runtime state before provider dispatch; retry-scheduled reconstruction remains non-dispatchable until current-claim settlement. Acknowledged delivery waits through that ordering, and startup settlement failure interrupts recovery before dispatch. Broader idempotent bootstrap and recovery remain incomplete |
