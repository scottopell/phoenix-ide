...
### REQ-STR-002 — Only Remove Recoverable Information

**Rationale:** The operator must be able to trust that nothing irreplaceable is
lost. A file read can be re-read; an answer the human typed cannot be
reconstructed. Removing the latter would silently corrupt the agent's reasoning.

IF a tool result could not be regenerated by re-invoking the tool that produced
it, THE SYSTEM SHALL NOT remove that result.
