...
A composer syntax for running Phoenix's worker tools directly as the user —
...
**Included:**
- A composer sigil for direct worker-tool invocation, distinct from `@`/`/`
  prose expansion
- Parse-time validation against the registry (unknown tool / not-user-invokable
  / bad arguments rejected before any effect runs)
- Worker-tools-only exposure (`bash`, `patch`, `keyword_search`, `read_image`,
  `think`); agent-control tools excluded
...
| Tool scope | Worker tools only | Agent-control tools are lifecycle plumbing, meaningless user-invoked |
...
| REQ-UTI-003: Worker tools only | Planned |
