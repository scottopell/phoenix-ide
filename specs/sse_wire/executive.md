...
1. **`SseBroadcaster`** (`crates/phoenix-ide/src/runtime.rs`) wraps a tokio broadcast channel plus a monotonic sequence counter (`AtomicI64`). Senders use either `next_seq()` to allocate or `send_persisted_message(...)` / `send_seq(...)` to allocate-and-emit. Persisted-message transactions reserve their sequence range from this broadcaster before writing rows; the reservation gates higher ephemeral broadcasts until the durable messages are emitted or the transaction fails. `observe_seq(seq)` folds an already-persisted sequence ID back into the counter so a later `next_seq()` is strictly greater. A pre-allocated sequence ID is the wire-level handle the client uses for replay-suppression and ordering.
