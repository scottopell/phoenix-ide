...
The event types in scope are: `init` (the snapshot delivered on subscribe), `message` (a newly persisted message), `message_updated` (a mutation to an existing message's fields), `state_change` (a phase transition), `token` (an LLM streaming chunk), `llm_first_byte` and `llm_attempt` (LLM request lifecycle markers), `agent_done` (turn completion), `conversation_hard_deleted` (row removed), `conversation_update` (partial metadata change), `product_conversation_lifecycle` (the replayable Open/History carrier), `continuation_boundary` (the durable predecessor/successor join carrier), `work_scope_update` (attached resource inventory), `browser_session_state` (browser-session liveness edge), `steer_message_queued` (a queued steering message), `rate_limit_snapshot` (mid-stream quota detail), and `error` (a user-facing error). Aggregate Open/History lifecycle is projected consistently as the authoritative lifecycle fact; there is no separate required row-terminal SSE event. Two ordering invariants are load-bearing: every delivered message event has a backing committed-to-DB row (PersistBeforeBroadcast), and `init` is always the first event on a stream (InitAlwaysFirst). Together they guarantee that a reconnecting client's fresh snapshot can never be behind a message that was already broadcast to a now-dead stream.
...
1. **`SseBroadcaster`** (`crates/phoenix-ide/src/runtime.rs`) wraps a tokio broadcast channel plus a monotonic sequence counter (`AtomicI64`). Senders use either `next_seq()` to allocate or `send_persisted_message(...)` / `send_seq(...)` to allocate-and-emit. `observe_seq(seq)` lets the persisted-message path fold the DB-allocated sequence ID back into the counter so a later `next_seq()` is strictly greater. For root product-conversation streams, this monotonic envelope sequence is authoritative for replay-suppression and ordering; the spec now makes that authority concrete as one RootStreamLedger consumed by init, replay, and every live aggregate-bound carrier. Member message sequence IDs remain row-local identity, not the root stream watermark.

2. **`sse_stream`** (`crates/phoenix-ide/src/api/sse.rs`) is the per-client handler that emits `init` first (a DB snapshot taken at subscribe time, including `last_sequence_id` and the ring's pending events), then forwards broadcast events. The same RootStreamLedger ordering must back both the init snapshot and the replay ring, and every aggregate-bound live carrier must materialize as one closed `AggregateBoundLiveEvent` envelope carrying the root ProductConversation id, the allocated `root_sequence_id`, and member conversation identity where applicable. `work_scope_update` is part of that aggregate-bound set: it allocates from the same root ledger, carries the root ProductConversation id, uses inventory payload identity as its envelope identity, and leaves `member_conversation_id` null because no transcript member owns the attached WorkScope row. Durable aggregate facts must remain available as typed replayable carriers: `continuation_boundary` for predecessor/successor joins and `product_conversation_lifecycle` for Open/History projection. Advancing the root stream sequence without a parseable typed payload would violate this contract. On `BroadcastStreamRecvError::Lagged` — the broadcast channel ring buffer wrapped because the client fell too far behind — the stream closes; the client's connection machine reconnects and the next `init` resyncs from the DB.
...
| **AggregateLifecycleProjectionBroadcast** | Typed replayable `product_conversation_lifecycle` carrier projected as the authoritative Open/History fact on the root conversation stream |
| **ConversationUpdateBroadcast** | Partial-metadata SSE event; client merges shallowly |
| **ContinuationBoundaryBroadcast** | Typed replayable `continuation_boundary` carrier for the durable predecessor/successor join |
| **AggregateBoundLiveCarrierEnvelopeAllocated** | Closed aggregate-bound live envelope over the authoritative carrier set; excludes only `init` and terminal `conversation_hard_deleted` |
| **AggregateBoundMemberLiveCarrierClaimsRootSequence** | Generic member-scoped producer contract for `state_change`, `token`, `message_updated`, `agent_done`, `conversation_update`, `error`, `browser_session_state`, `steer_message_queued`, `rate_limit_snapshot`, `llm_first_byte`, and `llm_attempt` |
