...
| **ProductConversationLifecycleBroadcast** | Aggregate Open/History lifecycle projection delivered on the root conversation stream |
...
- **`specs/bedrock/`**: the source of `state_change`, `agent_done`, and aggregate lifecycle projection events. Bedrock owns the state machine; this spec owns the wire envelope.
