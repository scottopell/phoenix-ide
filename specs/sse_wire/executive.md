...
2. **`sse_stream`** (`crates/phoenix-ide/src/api/sse.rs`) is the per-client handler that emits `init` first (a DB snapshot taken at subscribe time, including `last_sequence_id` and the ring's pending events), then forwards broadcast events. The same RootStreamLedger ordering must back both the init snapshot and the replay ring, and durable aggregate facts must be available as typed replayable carriers: `continuation_boundary` for predecessor/successor joins and `product_conversation_lifecycle` for Open/History projection. Advancing the root stream sequence without a parseable typed payload would violate this contract. On `BroadcastStreamRecvError::Lagged` — the broadcast channel ring buffer wrapped because the client fell too far behind — the stream closes; the client's connection machine reconnects and the next `init` resyncs from the DB.
