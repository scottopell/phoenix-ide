...
WHEN the applied diff exceeds the response's bounded line budget
THE SYSTEM SHALL truncate it
AND state how many lines were omitted so the agent knows the preview is partial
...
**Rationale:** Clear feedback helps agents verify edits. Returning the applied diff to the agent — not only to the UI — closes the verify-by-re-reading loop that otherwise follows every edit. Diffs enable user review. Autogenerated warnings prevent accidental modifications to generated code.
