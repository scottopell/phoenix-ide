...
## Merge Approval State (REQ-BED-029, REQ-PROJ-009)

```rust
AwaitingMergeApproval {
    task_id: String,
    diff_summary: String,   // generated by executor at state entry
    reply: oneshot::Sender<MergeApprovalOutcome>,
}

enum MergeApprovalOutcome {
    Approved,
...
AwaitingMergeApproval + Approved → Idle (mode becomes Explore)
    Effects: MergeWorktree, DeleteWorktree, UpdateTaskStatus(done), PersistMode

AwaitingMergeApproval + ChangesRequested → Idle (mode stays Work)
    Effects: (feedback returned to agent as message)
```
