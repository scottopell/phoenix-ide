...
AND released duplicate-sequence repairs SHALL preserve every row, order affected conversation partitions by durable `(old sequence_id, created_at, message_id)`, deterministically stable-shift affected sequence identities when required, allow repaired identities to remain sparse rather than dense, remap `clear_watermark` by stable cleared-row identity/order, and advance transcript generation for each affected conversation
...
AND SHALL make ProductConversation the authority for stable aggregate identity, aggregate membership, topology-derived canonical navigation, ordinary lifecycle, and aggregate presentation
AND SHALL keep continuation-edge topology, message persistence, SSE publication, runtime and provider sessions, and generation-fenced prompt projection scoped to transcript members
