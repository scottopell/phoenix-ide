...
describe('ConversationPage route focus', () => {
  it('restores composer focus when SSE resolves questions before the HTTP callback', async () => {
    ...
    act(() => store.dispatch(slug,{type:'sse_state_change',sequenceId:1,phase:{type:'awaiting_user_response',tool_use_id:'first-question',questions:[{header:'Pick',question:'Choose?',multiSelect:false,options:[{label:'A'},{label:'B'}]}]},stateUpdatedAt:Date.now()+1}));
  ...
  it('keeps a newer question focused when an earlier callback arrives', async () => {
    ...
    act(() => store.dispatch(slug,{type:'sse_state_change',sequenceId:1,phase:{type:'awaiting_user_response',tool_use_id:'first-question',questions},stateUpdatedAt:Date.now()+1}));
    await screen.findByRole('radio',{name:'A'});
    act(() => store.dispatch(slug,{type:'sse_state_change',sequenceId:2,phase:{type:'awaiting_user_response',tool_use_id:'second-question',questions},stateUpdatedAt:Date.now()+2}));
    act(() => store.dispatch(slug,{type:'question_phase_change',expectedConversationId:slug,toolUseId:'first-question',phase:{type:'idle'}}));
  ...
  it('does not steal focus when the route renders no composer', async () => {
    ...
    renderPage(makeConversation({
      state: { type: 'awaiting_user_response', tool_use_id: 'test-question', questions: [] },
    }));
