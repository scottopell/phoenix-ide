...
describe('conversationReducer', () => {
  ...
  describe('set_draft', () => {
    it('replaces the draft when expectedConversationId matches', () => {
      const atom: ConversationAtom = {
        ...createInitialAtom(),
        conversationId: 'conv-1',
        draft: 'old',
    ...
    it('drops when expectedConversationId does not match', () => {
      ...
      const next = dispatch(atom, {
        type: 'set_draft',
        text: 'late from conv-A',
        expectedConversationId: 'conv-A',
      });

      expect(next).toBe(atom);
      expect(next.draft).toBe('kept');
    });
  });
  ...
  describe('append_draft', () => {
    ...
    it('replaces (no separator) when existing draft is empty / whitespace-only', () => {
      const atom: ConversationAtom = {
        ...createInitialAtom(),
        conversationId: 'conv-1',
        draft: '   \n  ',
  ...
  describe('clear_draft', () => {
    it('empties the draft when expectedConversationId matches', () => {
      const atom: ConversationAtom = {
        ...createInitialAtom(),
        conversationId: 'conv-1',
        draft: 'something',
