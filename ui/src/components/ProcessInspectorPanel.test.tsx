...
async function renderPanel() {
  ...
  await act(async () => {
    utils = render(<ProcessInspectorPanel conversationId="conv-1" scopeKey="ws-1" controlScopeKey="ws-1" handleId="b-1" />);
...
describe('ProcessInspectorPanel — output accumulation', () => {
  it('seeds with no `since`, then polls with `since = end_offset`, appending lines', async () => {
    ...
    expect(getInsp).toHaveBeenLastCalledWith('ws-1', 'b-1', 'conv-1', 'ws-1');
    ...
    expect(getInsp).toHaveBeenLastCalledWith('ws-1', 'b-1', 'conv-1', 'ws-1', 2);
...
describe('ProcessInspectorPanel — poll serialization', () => {
  it('does not issue a second overlapping poll while one is in flight, and an out-of-order resolution cannot duplicate lines or regress the cursor', async () => {
    ...
    expect(getInsp).toHaveBeenLastCalledWith('ws-1', 'b-1', 'conv-1', 'ws-1', 1);
    ...
    expect(getInsp).toHaveBeenLastCalledWith('ws-1', 'b-1', 'conv-1', 'ws-1', 3);
  ...
  it('a late poll from a PRIOR target cannot clear the new target\'s in-flight gate (no overlapping fetch on the new target)', async () => {
    ...
    let utils!: ReturnType<typeof render>;
    await act(async () => {
      utils = render(<ProcessInspectorPanel conversationId="conv-1" scopeKey="ws-1" controlScopeKey="ws-1" handleId="a-1" />);
      await Promise.resolve();
    });
    ...
    expect(getInsp).toHaveBeenLastCalledWith('ws-1', 'a-1', 'conv-1', 'ws-1', 1);
    ...
    await act(async () => {
      utils.rerender(<ProcessInspectorPanel conversationId="conv-1" scopeKey="ws-1" controlScopeKey="ws-1" handleId="b-2" />);
      await Promise.resolve();
    });
    ...
    expect(getInsp).toHaveBeenLastCalledWith('ws-1', 'b-2', 'conv-1', 'ws-1', 1);
...
describe('ProcessInspectorPanel — poll failure surfacing', () => {
  ...
  it('retries a transient seed failure on the next interval and recovers to a normal snapshot', async () => {
    ...
    expect(getInsp).toHaveBeenLastCalledWith('ws-1', 'b-1', 'conv-1', 'ws-1', undefined);
