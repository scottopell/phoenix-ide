...
const question: UserQuestion = {header:'Choice',question:'Which?',multiSelect:true,options:[{label:'A'},{label:'B'}]};
describe('AUQ draft payload', () => {
  it('preserves multiline custom/notes, sorts selected labels in display order, and ignores deselected custom text', () => {
    ...
    expect(answerPayload([question],[draft])).toEqual({answers:{'Which?':'A, B, custom\nanswer'},annotations:{'Which?':{notes:'  exact notes\n  '}}});
    draft=choose(draft,2,false);
    expect(answerPayload([question],[draft]).answers).toEqual({'Which?':'A, B'});
  });
