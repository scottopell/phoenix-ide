...
const files = new Map<string, FileItem[]>([
  [GROUNDING_PANEL_ROOT, [
    ...
    dir('examples', `${GROUNDING_PANEL_ROOT}/examples`),
    ...
    file('root-file.ts', `${GROUNDING_PANEL_ROOT}/root-file.ts`),
  ]],
  ...
  [`${GROUNDING_PANEL_ROOT}/examples`, [dir('nested', `${GROUNDING_PANEL_ROOT}/examples/nested`), file('example-root.md', `${GROUNDING_PANEL_ROOT}/examples/example-root.md`)]],
  [`${GROUNDING_PANEL_ROOT}/examples/nested`, [file('child-file.ts', `${GROUNDING_PANEL_ROOT}/examples/nested/child-file.ts`)]],
...
const workScope: WorkScopeInventory = {
  ...
  bash: [
    { control_scope_key: 'work:fixture', handle_id: 'b-1', label: 'vite dev server', cmd: 'pnpm --dir ui dev', state: 'running', pid: 42100, pgid: 42100, started_at: new Date(Date.now() - 75_000).toISOString(), output_bytes: 1_200_000, health: { cpu_percent: 7.2, memory_bytes: 190_840_832, process_count: 3 } },
    { control_scope_key: 'work:fixture', handle_id: 'b-2', label: 'stress first-turn SSE coverage', cmd: 'for i in 1 2 3; do uv run tests/e2e/run.py || exit $?; done', state: 'kill_pending_kernel', pid: 42110, pgid: 42110, started_at: new Date(Date.now() - 185_000).toISOString(), output_bytes: 23_000, health: { cpu_percent: 241.9, memory_bytes: 1_298_939_904, process_count: 8 } },
    { control_scope_key: 'work:fixture', handle_id: 'b-3', label: 'seed fixture', cmd: './dev.py seed', state: 'tombstoned', started_at: new Date(Date.now() - 480_000).toISOString(), duration_ms: 34_000, exit_code: 0, output_bytes: 82_000 },
    { control_scope_key: 'work:fixture', handle_id: 'b-4', label: 'failing lint', cmd: 'pnpm lint', state: 'tombstoned', started_at: new Date(Date.now() - 360_000).toISOString(), duration_ms: 19_000, exit_code: 2, output_bytes: 18_000 },
  ],
  tmux: { status: 'live' },
